...
const ReferralTerminal: React.FC<ReferralTerminalProps> = ({ user, onUserUpdate, onClose }) => {
  const [copySuccess, setCopySuccess] = useState(false);
  const [isDispatching, setIsDispatching] = useState(false);
  const [claimStatus, setClaimStatus] = useState<'idle' | 'claiming' | 'success'>('idle');

  const referralLink = `${window.location.origin}/?ref=${user.nodeId}`;
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-0 sm:p-4 bg-black/95 backdrop-blur-3xl animate-in fade-in">
      <div className="bg-[#1e222d] w-full h-full sm:h-auto sm:max-w-xl sm:rounded-[3rem] border border-white/10 overflow-hidden shadow-[0_0_100px_rgba(0,0,0,1)] flex flex-col">
             ...
             <div className="relative z-10 space-y-4">
                ...
                <div className="pt-6 grid grid-cols-2 gap-3">
                   ...
                   <button 
                     ...
                     className={`py-5 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl active:scale-95 transition-all flex items-center justify-center gap-2 relative overflow-hidden ${
                        (user.pendingClaims || 0) > 0 
